...
game_parser.add_argument('text', type=str, required=True, help='Text cannot be blank')
game_parser.add_argument('font_photo', type=FileStorage, location='files', required=True, help='Font photo is required')
game_parser.add_argument('handwriting_photo', type=FileStorage, location='files', required=True, help='Handwriting photo is required')
@api.route('/game')
class Game(Resource):
    def post(self):
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=6259, ssl_context=('cert.pem', 'key.pem'))
